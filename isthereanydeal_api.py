...
class ITAD_API(GameSalesAPI):
    ...
    @staticmethod
    def fetch_game_sales(limit=1, stores=None, sort_by: ITAD_SortOptions = ITAD_SortOptions.PRICE_MOST_EXPENSIVE) -> list[SaleDetails]:
        params = {
            'key': ITAD_API._api_key,
            'limit': str(limit),
            'sort': sort_by.value[0],
        }

        if stores is not None:
            params['shops'] = str(stores)
